...
M1 CHIP DECISION — trigger: chips_remaining is not "none".
   For each remaining chip, the case for and against playing it
   THIS week, judged against the squad in the brief. Bench
   strength (the "Bench / XI" ratio and bench ep_next) for Bench
   Boost, captain fixture and is_double for Triple Captain, squad
   damage (Flags table, Status column) for Free Hit and Wildcard.
   Only one chip can be played per gameweek.

M2 SQUAD HEALTH — trigger: always. CHIP_INDEPENDENT.
   Start from the Flags table: the API's INJ/DOUBT/SUSP flags lag
   real news. Then max 3 web searches, last 3 days: injuries,
   press conference team news, expected lineups. Only players in
   my 15. Ignore general FPL content. For each doubt, say whether
   the bench covers it (same position, plays this GW) or a
   transfer is needed. Name the captain and vice from the
   candidates table, one line of reason.

M3 H2H CONTEXT — trigger: the H2H table has at least one row.
   For each league: my rank, my H2H points, this week's opponent,
   their rank, the points gap, their FPL total versus mine. State
   whether the matchup argues for aggression or for holding the
   chip. The brief does not include the opponent's squad; do not
   guess at it.

M4 EXPIRY PRESSURE — trigger: number of chips remaining >= gameweeks_to_expiry + 1 (gameweeks left including this one). CHIP_INDEPENDENT.
   Warn I am now forced into bad timing; sequence the remaining
   chips across the weeks left, one chip per gameweek, naming the
   gameweek for each. gameweeks_to_expiry of 0 means this is the
   last chance for every chip still remaining.

[To add a module: create modules/NN-name.md with frontmatter name,
 trigger, chip_independent, order; run `make routine`.]

=== STEP 2: OUTPUT ===
Verdict first: play a chip this week, yes/no, which.
Then modules in order. Under 400 words.
Trust the brief's numbers over your own arithmetic.
Then one row for state/decisions.md, ready to paste:
  | YYYY-MM-DD | GW<n> | <verdict, five words or fewer> | <reason, one sentence> |
Last line: modules skipped, and why.
